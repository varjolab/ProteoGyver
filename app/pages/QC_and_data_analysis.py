...
@callback(
    ...
    Output({'type': 'uploaded-data-store',
           'name': 'uploaded-mztab-sample-table'}, 'data'),
...
def handle_uploaded_data_table(
    ...
    if file_contents is not None:
        upload_style, info, tables, warning_list, mztab_stable = parsing.parse_data_file(
        ...
        if len(warnings) > 0:
            ...
            warnings.append(html.P('This might be due to file format. Supported formats are: csv (comma separated); tsv, txt, tab (tab separated); xlsx, xls (excel); mztab and sdrf support is experimental and easy to break at the moment.'))
        return_vals = [upload_style, info, tables, warnings, len(warnings)==0]
        if mztab_stable is not None:
            return_vals.append(mztab_stable)
        else:
            return_vals.append(no_update)
...
def handle_uploaded_sample_table(
    ...
    """
    if file_contents is not None:
        upload_style, info, table_data = parsing.parse_sample_table(file_contents, file_name, mod_date, current_upload_style, parameters['Sample table parsing']['SDRF'])
        ...
        if len(exp_cols_found) < 2:
            ...
            warnings = [
                ...
                html.P('This might be due to file format. Supported formats are: csv (comma separated); tsv, txt, tab (tab separated); xlsx, xls (excel); mztab and sdrf support is experimental and easy to break at the moment.')
            ]
        if 'sdrf warnings' in info:
            if len(warnings) == 0:
                warnings = [html.H3('Sample table warnings')]
            warnings.append(html.P('SDRF file errors:'))
            warnings.extend([
                html.P(f'- {w}') for w in info['sdrf warnings']
            ])
        return upload_style, info, table_data, warnings, len(warnings)==0
    elif sample_table_from_mztab is not None:
        info: dict = {
            'Data source': 'MzTab autogenerated sample table',
            'required columns found': ['sample name','sample group'],
            'Modified time': mod_date,
            'File name': file_name
        }
        current_upload_style['background-color'] = 'green'
        return current_upload_style, info, sample_table_from_mztab, [], True
