...
def _unwrap(x):
    ...
    :param x: tomlkit item or plain Python value.
    :returns: Plain Python value with nested structures unwrapped.
    """
...
def _get(doc: dict, path: list[str], default=None):
    """Traverse a nested dict along ``path`` returning a default if missing.

    :param doc: Root dictionary.
...
def _resolve(base: str, p):
    ...
    :param p: Path string or None.
    :returns: Absolute/normalized path string or None.
    """
    if p is None:
        return None
...
def load_pipeline_parameters(
    ...
    :param user_toml: User-provided TOML path.
    :param defaults_dir: Directory containing default TOMLs.
    :returns: Merged parameters dictionary.
    :raises KeyError: If workflow is not defined in the user TOML.
    :raises ValueError: If workflow is unsupported.
    """
...
def load_config(toml_path: str, default_toml_dir: Path | None = None) -> pipeline_batch.BatchConfig:
    ...
    :param toml_path: Path to user TOML file.
    :param default_toml_dir: Optional directory of default TOMLs; when provided,
        a fully expanded TOML is emitted next to the user TOML for transparency.
    :returns: Populated BatchConfig instance.
    """
    if default_toml_dir:
        new_toml_path = f'{toml_path}_autogenerated_full_parameters.toml'
