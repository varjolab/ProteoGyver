...
leg_dict: dict = {
    'qc': {
        ...
        'missing_values-plot': 'Missing vlaues are mostly a problem in proteomics and phosphoproteomics workflows. In interactomics, depending on your specific bait set, we can expect to see either very few or very many of them. In particular, if you include controls, e.g. GFP, in your analysis set (as you should), it will inevitably raise the number of missing values. Missing values in interactomics only matter, if comparing protein abundance between different baits.',
        ...
        'shared_id-plot': 'Shared identifications across samples. The color corresponds to the number of shared identifications between row and column as a proportion of proteins identified in the corresponding samples.',
    },
    'proteomics': {
