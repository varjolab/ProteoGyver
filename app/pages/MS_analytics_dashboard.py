...
def update_tic_graph(_,prev_btn_nclicks, next_btn_nclicks, __, tic_index: int, ticlist:list, datatype:str, supp_datatype:str, traces:dict, plot_data: str, max_y:dict) -> tuple:
    supp_datatype = supp_datatype.lower()
    data_to_use: pd.DataFrame = pd.read_json(plot_data,orient='split')
    datatype = datatype.lower()
    traces = {int(k): v for k,v in traces.items()} # Trace dict int type keys will be converted to strings when passing through dcc store
    ticlist.sort()
    next_offset: int = 0
...
def update_run_choices(_, start, end, sample_types, run_id_list, button_text) -> list:
    if (run_id_list is None ) or (run_id_list.strip() == ''):
        ...
        chosen_runs.index = chosen_runs.index.astype(int)# Need to do this flip in order to sort it correctly
        chosen_runs.sort_index(ascending=True, inplace=True)
