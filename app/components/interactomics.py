...
def count_knowns(saint_output, replicate_colors) -> pd.DataFrame:
    data: pd.DataFrame = saint_output[['Bait', 'Known interaction']].\
        value_counts().to_frame().reset_index().rename(columns={0: 'Prey count'})
    color_col: list = []
    for _, row in data.iterrows():
        if row['Known interaction']:
            color_col.append(replicate_colors['contaminant']['sample groups'][row['Bait']])
        else:
            color_col.append(replicate_colors['non-contaminant']['sample groups'][row['Bait']])
...
def known_plot(filtered_saint_input_json, db_file, rep_colors_with_cont, figure_defaults, isoform_agnostic=False) -> tuple:
    ...
    if isoform_agnostic:
        upid_a_col+='_noiso'
        upid_b_col+='_noiso'
    saint_output: pd.DataFrame = pd.read_json(filtered_saint_input_json, orient='split')
    ...
    saint_output = pd.merge(
        ...
        db_functions.get_from_table_by_list_criteria(
            db_conn, 'known_interactions', upid_a_col,list(saint_output['Bait'].unique())
        ),
        left_on=['Bait','Prey'],
    ...
    col_order.append('Following columns are information about known interactions.')
    ...
    figure_data: pd.DataFrame = count_knowns(saint_output, rep_colors_with_cont)
    ...
    return (
        html.Div(
            ...
            children = [
                html.H4(id='interactomics-known-header',children='Identified known interactions'),
                ...
                legends['known'],
                html.P(f'Known counts per bait: {", ".join([b + ": " + str(figure_data.loc[b]["Prey count"]) for b in figure_data.index])}')
                ]
        ),
...
def pca(saint_output_data: dict, defaults: dict) -> tuple:
    ...
    data_table = data_table.pivot_table(index='Prey',columns='Bait',values='AvgSpec')
    pc1: str
    pc2: str
    pca_result: pd.DataFrame
    # Compute PCA of the data
    spoofed_sample_groups: dict = {i: i for i in data_table.columns}
    pc1, pc2, pca_result = matrix_functions.do_pca(data_table.fillna(0), spoofed_sample_groups, n_components = 2) 
...
def enrich(saint_output_json:str, chosen_enrichments:list, figure_defaults, sig_threshold: float = 0.01) -> tuple:
    ...
    for i, (rescol, sigcol, namecol, result) in enumerate(enrichment_results):
        ...
        keep_these = keep_these & set(result[result[sigcol]<sig_threshold][namecol].values)
...
def map_intensity(saint_output_json:str, intensity_table_json: str, sample_groups: dict) -> list:
    intensity_table: pd.DataFrame = pd.read_json(intensity_table_json,orient='split')
    saint_output: pd.DataFrame = pd.read_json(saint_output_json,orient='split')
    if intensity_table.columns[0] == 'No data' and len(intensity_table.columns)==1:
        intensity_column = [np.nan for _ in saint_output.index]
    else:
...
def add_bait_column(saint_output, bait_uniprot_dict) -> pd.DataFrame:
    saint_output['Bait uniprot'] = [bait_uniprot_dict[bait] for bait in saint_output['Bait'].values]
    return saint_output
...
def run_saint(saint_input: dict, saint_path:list, error_log_file: str, session_uid:str, bait_uniprots:dict, cleanup: bool = True) -> str:
    ...
    if failed:
        ...
        ret = add_bait_column(pd.read_csv(os.path.join(temp_dir, 'list.txt'), sep='\t'),bait_uniprots).to_json(orient='split')
        if cleanup:     
            try:
