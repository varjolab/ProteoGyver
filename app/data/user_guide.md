...
###### Imputation choice
Use QRILC. It's currently the best imputation method available. Only change this is you know why you're doing it.
###### Normalization choice
Median works well for most situations. If the post-normalization value distribution of your data looks like there are large differences between sample groups, try vsn or quantile.  
###### Comparisons
...
## Windowmaker
Windowmaker is a utility for generating DIAPasef windows based on either .mgf files (generated by timstof via data analysis from DDA runs) or spectral libraries (generated from DDA files). In-silico spectral libraries from DIA-NN are not compatible, and are not a good idea to use to begin with.
### Parameters:
- Checkbox to play notification sound when done: The process of window generation will take a WHILE. Especially with larger libraries. So windowmaker can play a helpful notification sound when the DIAPasef method is ready for download 
- These should be set based on instrument settings and settings you plan to use when searching the files with DIA-NN etc:
	- mz range
	- Ion mobility range
	- Peptide Range
- Line equation based filters:
	- These can be used to exclude ions under or over a given threshold represented by a line, e.g. 2 * x + 10.
	- Each line will be drawn on the plot after. The coordinate system is not based on IM or MZ values, but rather steps from the origo. Try it out, it should be intuitive.
- Filters:
	- These are automatically generated from the MGF file or spectral library. They include any value, that has 15 or fewer unique values across all ions. 
	- Common filters include charge and PTMs (if available in the input library)
### Notes
Running is directly proportional to the size of the input file and can take a while. Be patient.
