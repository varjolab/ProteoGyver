...
leg_dict: dict = {
    'qc': {
        ...
        'missing_values-plot': 'Missing values are mostly a problem in proteomics workflow. In interactomics, depending on your specific bait set, we can expect to see either very few or very many of them. In particular, if you include controls, e.g. GFP, in your analysis set (as you should), it will inevitably raise the number of missing values. Missing values in interactomics only matter, if comparing protein abundance between different baits.',
    ...
    'proteomics': {
        ...
        'clustermap': 'Correlation is another metric for how similar samples are. Similarly to above, samples should form clusters of each sample group. Similarity here is also not an absolute metric, or intended to capture more than a vague similarity between samples.',
    ...
    'interactomics': {
        ...
        'filtered-saint-counts': 'Preys have been filtered based on the selected thresholds. BFDR threshold means anything with the selected value or lower passes the filter. Preys that passed through the filter in at least one bait were $RESCUE from other baits. Bait self-interactions have been discarded, IF bait uniprots are in the input file.',
