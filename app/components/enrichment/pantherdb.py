...
class handler():
    ...
    def enrich(self, data_lists: list, options: str) -> list:
        ...
        results: dict = {}
        legends: dict = {}
        logger.debug(f'Enrich: {len(datasets)}')
        for bait, preylist in data_lists:
            logger.debug(f'Enrich for {bait}: {len(preylist)}')
            ...
            logger.debug(f'Enrich for {bait}: done')
        result_names: list = []
    ...
    def run_panther_overrepresentation_analysis(self, datasets: list, protein_list: list, data_set_name: str,
        ...
        for annotation, name, description in datasets:
            ...
            success: bool = False
            logger.debug(f'Run enrichment: {data_set_name} {name}')
            for i in range(20, 100, 20):
                try:
                    ...
                    success = True
                    logger.debug(f'Run enrichment: success')
                    break
                except requests.exceptions.ReadTimeout as e:
                    logger.debug(f'Run enrichment: fail-ReadTimeOut - {e}')
                    continue
                except requests.exceptions.ConnectionError as e:
                    logger.debug(f'Run enrichment: fail-ConnectionError - {e}')
                    continue
            if not success:
            ...
            try:
                ...
                logger.debug(f'Run enrichment: fail-KeyError - {exc}')
                raise exc
            reference_string += (
